...
const AddAddress: React.FC = (props: any) => {
	...
	useEffect(() => {
		axios.get('/settings/map').then((res) => {
			...
			setApiKey(apiKey)
			loadScript('https://maps.googleapis.com/maps/api/js?libraries=places&key=' + apiKey, 'googleMap', () => setLoaded(true))
		})
	}, [])
	...
	const reverseGeoCoder = async (lat: number, lng: number) => {
		await fetch('https://maps.googleapis.com/maps/api/geocode/json?address=' + lat + ',' + lng + '&key=' + apiKey)
	...
	const submitAddress = () => {
		...
		if (full_address.trim() === '' || label.trim() === '')
			return
		const { lat, lng } = center

		axios.post(`/user/address`, { full_address, label, lat, lng })
			.then(res => {
				const data = res.data
							...
							<Row className="my-0">
								<Col lg="5" className="p-0">
									<Row className="m-0">
												...
												{({ getInputProps, suggestions, getSuggestionItemProps, loading }) => (
													<div className="w-100 py-2 bg-white shadow-sm">
														<InputGroup className="input-group-alternative m-0 shadow-none">
															...
															{(!(props.history.location.state && props.location.state.redirectTo))
																&& (
																	<InputGroupAddon addonType="prepend" onClick={() => props.history.goBack()}>
																		<InputGroupText>
																			<IonIcon icon={arrowBackOutline} className="text-muted" />
																		</InputGroupText>
																	</InputGroupAddon>
																)}
														...
														<div className="autocomplete-dropdown-container bg-white">
															{suggestions.map(suggestion => {
																const className = 'suggestion-item w-100 p-2 border-bottom';
																return (
																	<div
																	...
																	>
																		<h6 className="my-0 font-weight-bold item-name">{suggestion.formattedSuggestion.mainText}</h6>
										...
										<h6 className="p-3 font-weight-weight text-warning w-100" onClick={e => getCurrentPosition()}>
											<IonIcon icon={locateOutline} className="text-warning mr-1 align-text-top" />Use Current Location
									</h6>
									</Row>
						...
						(
							...
							<Container fluid={true} className="m-0 p-0">
								<Card className="bg-white border-0">
									<GoogleMapComponent
										containerElement={<div style={{ height: `50vh`, position: 'relative' }} />}
									...
									<CardBody className="py-2 px-0 pb-5 card-body rounded bg-white">
										<Form role="form">
											<FormGroup className="m-0 border-bottom px-3 bg-white pt-3">
												<h6 className="m-0 font-weight-bold item-name">Current Marked Location</h6>
												<Input placeholder="Your Current Marked Location" value={searchAddress} disabled className="border-0 shadow-0 p-0 bg-white font-weight-bold" type="text" />
											</FormGroup>
											<FormGroup className="m-0 border-bottom p-3 bg-white">
												<h6 className="m-0 font-weight-bold item-name">Complete Address <span className="text-warning">*</span> </h6>
												<Input placeholder="Your Full address" className="border-0 shadow-0 p-0" type="text" onChange={(e) => { setUserAddress({ ...userAddress, full_address: e.target.value }) }} value={userAddress.full_address} />
											</FormGroup>
											<FormGroup className="m-0 p-3 bg-white">
												<h6 className="m-0 font-weight-bold item-name">Tag this location <span className="text-warning">*</span></h6>
												<Input placeholder="Like Home/Office/Work" className="border-0 shadow-0 p-0" type="text" onChange={(e) => { setUserAddress({ ...userAddress, label: e.target.value }) }} value={userAddress.label} />
											</FormGroup>
								...
								<Col xs="3" className="my-4 fixed-top bg-secondary text-center shadow rounded-pill pr-1 ml-n4" onClick={() => setMapEnabled(false)}>
									<h2 className="m-0">
										<IonIcon icon={arrowBackOutline} className="text-muted align-text-top" />
									</h2>
								</Col>
							</Container>
