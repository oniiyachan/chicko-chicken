...
const Cart: React.FC = (props: any) => {
	...
	useEffect(() => {
		...
		axios.get(`/cart/${basket_restaurant_id}?t=${new Date().getTime()}`)
			.then(res => {
				const data = res.data
				setData({
			...
			<IonHeader className="bg-white ion-no-border border-bottom">
				<IonToolbar color="white">
					...
					<IonTitle className="p-0">
						<small className="font-weight-bold">{(data.details.name) ?? 'Cart'}</small>
					</IonTitle>
				</IonToolbar>
			</IonHeader>
					...
					(
						<Container fluid={true} className="h-100 d-flex align-items-center">
							<Col>
								...
								<img src={require('../../assets/img/icons/common/pan-toss.png').default}  className="p-4 w-100" alt="pan" />
